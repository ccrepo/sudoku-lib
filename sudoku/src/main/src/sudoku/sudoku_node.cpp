...
std::atomic<uint64_t> Node::_node_ids(0);
...
bool Node::do_setup_move(const Move& move) {

  if (!_board->do_move(move)) {
    return false;
  }
...
uint16_t Node::do_depth_first_search(Node *node,
  ...
  for (uint16_t i(0); i < possible_moves.size(); ++i) {
    ...
    if (child_node->do_move(possible_move)) {
      ...
      if (child_node->is_board_invalid()) {

        LOGTRACE("continue[board invalid] depth ",
            node->get_depth(),
            ".",
            (i+1),
            "/",
      ...
      if (result == C_NODE_DFS_CODE_OK) {
        ...
        if (child_node->is_board_invalid()) {
          ...
          LOGTRACE("continue[board invalid] from depth ",
              node->get_depth(),
              ".",
              (i + 1),
              "/",
