...
std::atomic<uint64_t> Board::_board_ids(0);

std::string Board::to_string(bool include_header [[maybe_unused]]) const {

  static const bool is_high_level_logging(
  ...
  os << std::endl
      ...
      << _cells.to_string()
      << std::endl
      << _cells.to_string(is_high_level_logging);
  ...
  if (is_high_level_logging) {

    os << std::endl
        << std::endl
        << " [ ";

    for (size_t i(0); i < _cells.get_cells().size(); ++i) {
...
bool Board::is_move_possible(const Move& move) const {
  ...
  if (is_invalid()) {
    LOGERROR("invalid board ", _board_id);
    return false;
  }
...
Board::Board() : _board_id(++_board_ids),
		    _parent_board_id(0),
		    _depth(1),
		    _cells { },
		    _invalid(false),
