...
public:
  ...
  bool do_setup_move(const Move& move);
  ...
  inline bool is_board_invalid() const {
    return _board->is_invalid();
  }
...
private:
  ...
  inline bool do_move(const Move& move) {
    return _board->do_move(move);
  }
...
private:
  static std::atomic<uint64_t> _node_ids;
