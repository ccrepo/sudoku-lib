...
public:
  virtual bool do_lock() override {
    return _is_locked.exchange(true);
  }
...
private:
  std::atomic<bool> _is_interrupted;

  std::atomic<bool> _is_locked;

  std::atomic<bool> _is_processed;

private:
  static std::atomic<uint64_t> _game_ids;
