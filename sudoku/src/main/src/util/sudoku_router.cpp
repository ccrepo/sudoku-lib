...
bool Router::route_round_robin(const std::shared_ptr<WorkI>& work, bool retry) {
  auto index(_round_robin_route_counter.
      fetch_add(1)%WorkerPool::C_WORKERPOOL_WORKERS_POOL_TOTAL);
...
bool Router::route_load_balance(const std::shared_ptr<WorkI>& work,
  ...
  std::sort(worker_pool_loads.begin(), worker_pool_loads.end(),
  ...
  return _worker_pools[worker_pool_loads[0].first]->push(work, retry);
}
