...
int32_t get_sudoku_possible_moves_STUB(const char* in_request,
    uint8_t* out_response,
    int32_t* out_runtime) {

  if (!Common::check_parameters(in_request,
      out_response,
      out_runtime)) {
  ...
  try {
    ...
    PossibleMoves possible_moves(in_request);

    uint32_t result (possible_moves.get_sudoku_possible_moves(moves_buffer,
        *out_runtime));
    ...
    os << Xml::get_root_tag_open()
        ...
        << Xml::get_runtime_xml(*out_runtime)
